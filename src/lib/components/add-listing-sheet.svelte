<script lang="ts">
	...
	import { addListing } from '$lib/stores/listings';
	...
	let { currentUserEmail = '' }: { currentUserEmail?: string } = $props();
	...
	let plotArea = $state('');
	let builtUpArea = $state('');
	let grossFloorArea = $state('');
	...
	let buyingPrice = $state('');
	let liquidityInvested = $state('');
	let sellingPrice = $state('');
	let listedByEmails = $state<string[]>([currentUserEmail || '']);
	let mediaAssets = $state<
		{ id: number; type: 'photo' | 'video'; fileName: string; previewUrl?: string }[]
	>([]);
	...
	function validate() {
		...
		if (!buyingPrice.trim()) nextErrors.buyingPrice = 'Buying price is required';
		if (!liquidityInvested.trim()) nextErrors.liquidityInvested = 'Liquidity invested is required';
		if (!sellingPrice.trim()) nextErrors.sellingPrice = 'Selling price is required';
		...
		if (propertyType === 'apartment') {
			if (!propertySize.trim()) nextErrors.propertySize = 'Property size is required for apartment';
		}
		if (propertyType === 'townhouse' || propertyType === 'villa') {
			if (!plotArea.trim()) nextErrors.plotArea = 'Plot area is required';
			if (!builtUpArea.trim()) nextErrors.builtUpArea = 'Built up area is required';
		}
		if (propertyType === 'commercial') {
			if (!propertySize.trim()) nextErrors.propertySize = 'Property size is required';
			if (commercialSubType === 'warehouse' && !grossFloorArea.trim()) {
				nextErrors.grossFloorArea = 'Gross floor area is required for warehouse';
			}
		}
		if (propertyType === 'plot' && !plotArea.trim()) {
			nextErrors.plotArea = 'Plot area is required for plot';
		}
		if (listingType === 'portal') {
			if (!titleDeedFileName) nextErrors.titleDeedFileName = 'Title deed/Qood is required for portal listing';
			if (!passportFileName) nextErrors.passportFileName = 'Passport is required for portal listing';
			if (!emiratesIdFileName) nextErrors.emiratesIdFileName = 'Emirates ID is required for portal listing';
		}
	...
	function saveListing() {
		...
		const listing: Listing = {
			id: `LST-${now.getTime()}`,
			listingType,
			listedByEmails: listedByEmails.map((value) => value.trim()).filter(Boolean),
			mediaAssets: mediaAssets
				.filter((asset) => asset.fileName.trim())
				.map((asset) => ({ type: asset.type, fileName: asset.fileName.trim() })),
			propertyAddress: {
			...
			unitNo: unitNo.trim(),
			propertyType,
			bedroomType: propertyType === 'apartment' || propertyType === 'townhouse' || propertyType === 'villa' ? bedroomType : undefined,
			commercialSubType: propertyType === 'commercial' ? commercialSubType : undefined,
			...
			titleDeedFileName: titleDeedFileName || undefined,
			passportFileName: passportFileName || undefined,
			emiratesIdFileName: emiratesIdFileName || undefined,
			buyingPrice: Number(buyingPrice),
			liquidityInvested: Number(liquidityInvested),
...
<Sheet.Root bind:open={open} onOpenChange={handleOpenChange}>
	<Sheet.Trigger>
		{#snippet child({ props })}
	...
	<Sheet.Content side="right" class="w-200 max-w-200 overflow-y-auto sm:w-200 sm:max-w-200">
		<div class="sticky top-0 z-10 flex items-center justify-between border-b bg-background p-6">
			...
			<div class="flex flex-row gap-2">
				...
				<Button type="button" size="sm" onclick={saveListing}>
		...
		<div class="flex flex-col gap-8 p-6">
				...
				<Field.Group>
					<div class="grid grid-cols-3 gap-4">
						<Field.Field>
							...
							{#if errors.firstName}<Field.Error class="text-sm text-destructive">{errors.firstName}</Field.Error>{/if}
						</Field.Field>
						<Field.Field>
							<Input bind:value={lastName} placeholder="Last Name" />
							{#if errors.lastName}<Field.Error class="text-sm text-destructive">{errors.lastName}</Field.Error>{/if}
						</Field.Field>
						<Field.Field>
							<Input type="email" bind:value={clientEmail} placeholder="Email" />
							{#if errors.clientEmail}<Field.Error class="text-sm text-destructive">{errors.clientEmail}</Field.Error>{/if}
						</Field.Field>
					</div>
					<Field.Field>
						<Field.Label>Phone Number</Field.Label>
						<Input bind:value={clientPhone} placeholder="Enter a phone number" />
						{#if errors.clientPhone}<Field.Error class="text-sm text-destructive">{errors.clientPhone}</Field.Error>{/if}
					</Field.Field>
				</Field.Group>
			</Field.Set>
				...
				<Field.Group>
					<div class="grid grid-cols-2 gap-4">
						<Field.Field>
							...
							{#if errors.developer}<Field.Error class="text-sm text-destructive">{errors.developer}</Field.Error>{/if}
						</Field.Field>
						<Field.Field>
							<Field.Label>Community <span class="text-muted-foreground">(Optional)</span></Field.Label>
						...
						<Field.Field>
							...
							<InputGroup.Root id="project">
								<InputGroup.Input bind:value={project} placeholder="Select Project" />
								<InputGroup.Addon>
									<Building />
								</InputGroup.Addon>
							</InputGroup.Root>
							{#if errors.project}<Field.Error class="text-sm text-destructive">{errors.project}</Field.Error>{/if}
						</Field.Field>
						<Field.Field>
							<Field.Label>Unit No</Field.Label>
							<InputGroup.Root id="unitNo">
								<InputGroup.Input bind:value={unitNo} placeholder="Unit No" />
								<InputGroup.Addon>
									<Home />
								</InputGroup.Addon>
							</InputGroup.Root>
							{#if errors.unitNo}<Field.Error class="text-sm text-destructive">{errors.unitNo}</Field.Error>{/if}
						</Field.Field>
						<Field.Field>
							<Field.Label>Property Type</Field.Label>
							<select class="h-10 w-full rounded-md border border-input bg-transparent px-3 text-sm" bind:value={propertyType}>
						...
						{#if propertyType === 'commercial'}
							<Field.Field>
								...
								<select class="h-10 w-full rounded-md border border-input bg-transparent px-3 text-sm" bind:value={commercialSubType}>
						...
						{#if propertyType === 'apartment'}
							<Field.Field>
								...
								<select class="h-10 w-full rounded-md border border-input bg-transparent px-3 text-sm" bind:value={bedroomType}>
							...
							<Field.Field>
								...
								<Input type="number" bind:value={propertySize} />
								{#if errors.propertySize}<Field.Error class="text-sm text-destructive">{errors.propertySize}</Field.Error>{/if}
							</Field.Field>
						{/if}
						...
						{#if propertyType === 'townhouse' || propertyType === 'villa'}
							...
							<Field.Field>
								...
								<Input type="number" bind:value={plotArea} />
								{#if errors.plotArea}<Field.Error class="text-sm text-destructive">{errors.plotArea}</Field.Error>{/if}
							</Field.Field>
							<Field.Field>
								<Field.Label>Built Up Area</Field.Label>
								<Input type="number" bind:value={builtUpArea} />
								{#if errors.builtUpArea}<Field.Error class="text-sm text-destructive">{errors.builtUpArea}</Field.Error>{/if}
							</Field.Field>
						{/if}
						...
						{#if propertyType === 'commercial'}
							...
							{#if commercialSubType === 'warehouse'}
								<Field.Field>
									...
									<Input type="number" bind:value={grossFloorArea} />
									{#if errors.grossFloorArea}<Field.Error class="text-sm text-destructive">{errors.grossFloorArea}</Field.Error>{/if}
								</Field.Field>
							{/if}
						{/if}
				...
				<Field.Group>
					<div class="grid grid-cols-2 gap-4">
						<Field.Field>
							...
							<Input bind:value={addressLine1} placeholder="Address line 1" />
						</Field.Field>
						<Field.Field>
							<Field.Label>Address Line 2</Field.Label>
							<Input bind:value={addressLine2} placeholder="Address line 2" />
						</Field.Field>
						<Field.Field>
							<Field.Label>Building Name</Field.Label>
							<Input bind:value={buildingName} placeholder="Building / Tower" />
						</Field.Field>
						<Field.Field>
						...
						<Field.Field>
							...
							<Input bind:value={area} placeholder="Area / Locality" />
						</Field.Field>
						<Field.Field>
						...
						<Field.Field>
							...
							<Input bind:value={postalCode} placeholder="Postal code" />
						</Field.Field>
						<Field.Field>
							<Field.Label>Landmark</Field.Label>
							<Input bind:value={landmark} placeholder="Nearby landmark" />
						</Field.Field>
					</div>
				</Field.Group>
			</Field.Set>
			...
			<Field.Set>
				<Field.Legend class="flex items-center gap-4 text-lg font-medium">
					<div class="grid h-8 w-8 place-items-center rounded-lg border border-white/5 bg-orange-100 p-0">
				...
				<Field.Group class="space-y-4">
					<div class="grid gap-4 xl:grid-cols-2">
						<div class="flex items-center gap-4">
							<span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-orange-100 text-sm font-semibold text-orange-500">1</span>
							<Field.Field class="w-full">
								{#if titleDeedFileName}
									<div class="flex w-full items-center justify-between gap-3 rounded-lg border border-muted-foreground/40 bg-background p-3">
										...
										<button type="button" onclick={() => removeFile('titleDeed')} class="text-destructive hover:text-destructive/80">
											<Trash2 class="h-5 w-5" />
										</button>
									</div>
								{:else}
									<label for="titleDeed" class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-muted-foreground/40 bg-muted/20 p-2 text-lg font-semibold text-foreground transition hover:border-foreground/60">
										<Upload class="h-5 w-5 text-gray-600" />
										<span class="text-sm font-medium">Upload Title Deed / Qood</span>
									</label>
								{/if}
								<Input id="titleDeed" class="sr-only" type="file" onchange={(event) => onFileSelect(event, 'titleDeed')} />
								{#if listingType === 'portal'}
									<p class="text-xs text-muted-foreground">Required for portal listing</p>
								{/if}
								{#if errors.titleDeedFileName}<Field.Error class="text-sm text-destructive">{errors.titleDeedFileName}</Field.Error>{/if}
							</Field.Field>
						</div>

						<div class="flex items-center gap-4">
							<span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-orange-100 text-sm font-semibold text-orange-500">2</span>
							<Field.Field class="w-full">
								{#if passportFileName}
									<div class="flex w-full items-center justify-between gap-3 rounded-lg border border-muted-foreground/40 bg-background p-3">
										<div class="flex items-center gap-3">
										...
										<button type="button" onclick={() => removeFile('passport')} class="text-destructive hover:text-destructive/80">
											<Trash2 class="h-5 w-5" />
										</button>
									</div>
								{:else}
									<label for="passport" class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-muted-foreground/40 bg-muted/20 p-2 text-lg font-semibold text-foreground transition hover:border-foreground/60">
										<Upload class="h-5 w-5 text-gray-600" />
										<span class="text-sm font-medium">Upload Passport</span>
									</label>
								{/if}
								<Input id="passport" class="sr-only" type="file" onchange={(event) => onFileSelect(event, 'passport')} />
								{#if listingType === 'portal'}
									<p class="text-xs text-muted-foreground">Required for portal listing</p>
								{/if}
								{#if errors.passportFileName}<Field.Error class="text-sm text-destructive">{errors.passportFileName}</Field.Error>{/if}
							</Field.Field>
						</div>

						<div class="flex items-center gap-4">
							<span class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-orange-100 text-sm font-semibold text-orange-500">3</span>
							<Field.Field class="w-full">
								{#if emiratesIdFileName}
									<div class="flex w-full items-center justify-between gap-3 rounded-lg border border-muted-foreground/40 bg-background p-3">
										...
										<button type="button" onclick={() => removeFile('emiratesId')} class="text-destructive hover:text-destructive/80">
											<Trash2 class="h-5 w-5" />
										</button>
									</div>
								{:else}
									<label for="emiratesId" class="flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-muted-foreground/40 bg-muted/20 p-2 text-lg font-semibold text-foreground transition hover:border-foreground/60">
										<Upload class="h-5 w-5 text-gray-600" />
										<span class="text-sm font-medium">Upload Emirates ID</span>
									</label>
								{/if}
								<Input id="emiratesId" class="sr-only" type="file" onchange={(event) => onFileSelect(event, 'emiratesId')} />
								{#if listingType === 'portal'}
									<p class="text-xs text-muted-foreground">Required for portal listing</p>
								{/if}
								{#if errors.emiratesIdFileName}<Field.Error class="text-sm text-destructive">{errors.emiratesIdFileName}</Field.Error>{/if}
							</Field.Field>
						</div>
				...
				<Field.Group>
					<div class="grid grid-cols-3 gap-4">
						<Field.Field>
							...
							{#if errors.buyingPrice}<Field.Error class="text-sm text-destructive">{errors.buyingPrice}</Field.Error>{/if}
						</Field.Field>
						<Field.Field>
							<Field.Label>Liquidity Invested</Field.Label>
							<Input type="number" bind:value={liquidityInvested} placeholder="Liquidity Invested" />
							{#if errors.liquidityInvested}<Field.Error class="text-sm text-destructive">{errors.liquidityInvested}</Field.Error>{/if}
						</Field.Field>
						<Field.Field>
							<Field.Label>Selling Price</Field.Label>
							<Input type="number" bind:value={sellingPrice} placeholder="Selling Price" />
							{#if errors.sellingPrice}<Field.Error class="text-sm text-destructive">{errors.sellingPrice}</Field.Error>{/if}
						</Field.Field>
					</div>
				</Field.Group>
			</Field.Set>
