<script lang="ts">
	...
	const resetDraftState = () => {
		...
		createSale.fields.goAmlStatus?.set(undefined);
		jointBuyerFiles = {};
								...
								<div class="grid gap-4 xl:grid-cols-2">
									...
									<div class="col-span-2 flex w-full flex-col items-center gap-4">
										...
										{#if uploadedFiles.goAmlFormFile}
											...
											>
												<div class="flex items-center gap-3">
													...
													<div class="flex flex-col">
														<span class="text-sm font-medium"
															>{uploadedFiles.goAmlFormFile.name}</span
														>
														<span class="text-xs text-muted-foreground"
											...
											<div class="flex w-full items-center gap-3">
												...
												<button
													type="button"
													class="rounded-full border px-4 py-1.5 text-sm font-medium transition {createSale.fields.goAmlStatus?.value() ===
													'red-flag'
														? 'border-red-600 bg-red-100 text-red-700'
														: 'border-muted-foreground/40 text-muted-foreground hover:border-red-400 hover:text-red-600'}"
													onclick={() => createSale.fields.goAmlStatus?.set('red-flag')}
												>
													Red Flag
												</button>
												<button
													...
													onclick={() => createSale.fields.goAmlStatus?.set('green-flag')}
												>
													Green Flag
												</button>
											</div>
										...
										<Input
											id="goAmlFormFile"
											class="sr-only"
											{...createSale.fields.goAmlFormFile?.as('file')}
											files={undefined}
											accept="application/pdf"
											onchange={(e) => handleFileUpload('goAmlFormFile', e)}
											disabled={!canUploadManually}
										/>
										<input type="hidden" {...createSale.fields.goAmlStatus?.as('text')} />
										{#each createSale.fields.goAmlFormFile?.issues() ?? [] as issue, i (i)}
											<Field.Error class="text-sm text-destructive">{issue.message}</Field.Error>
										{/each}
										{#each createSale.fields.goAmlStatus?.issues() ?? [] as issue, i (i)}
											<Field.Error class="text-sm text-destructive">{issue.message}</Field.Error>
										{/each}
									</div>
								</div>
